...
public class DriveSubsystem extends Subsystem {
	...
	private SwerveModule createModule(final int driveId, final int rotateId, final String keyName) {
		...
		return new SwerveModule(new TalonSRX(driveId), new TalonSRX(rotateId), zeroPosition);
	}
	...
	public void _runTestAccel() {
		...
		if (_m_logFile != null) {
			try {
				_m_logFile.write(String.format("%f, %f\n", _m_timer.get(), m_module2.getDrivePosition()));
			} catch (IOException e) {
