package team578.lib.swerve.chilly;
...
// https://github.com/MTHSRoboticsClub/FRC-2019/blob/master/ChillySwerveTest/java/src/main/java/frc/team1778/ChillySwerve/ChillySwerveUnit.java#L198
...
public class ChillySwerveUnit {
	...
	public ChillySwerveUnit(int driveTalonID, int turnTalonID) {
		...
		driveMotor = configureMotor(driveTalonID, REVERSE_DRIVE_MOTOR, drive_kP, drive_kI, drive_kD, drive_kF,
				drive_kIZone);
		driveMotor.configSelectedFeedbackSensor(FeedbackDevice.QuadEncoder, PIDLOOP_IDX, TIMEOUT_MS); // am-3314a quad
																										// encoder
	...
	private WPI_TalonSRX configureMotor(int talonID, boolean revMotor, double pCoeff, double iCoeff, double dCoeff,
			double fCoeff, int iZone) {
		WPI_TalonSRX _talon;
		_talon = new WPI_TalonSRX(talonID);
